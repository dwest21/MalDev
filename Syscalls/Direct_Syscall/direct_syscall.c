...
int DirectSyscall(DWORD pid, PBYTE shell, SIZE_T shell_size);
...
int main(int argc, char* argv[]) {
    ...
    if (!DirectSyscall(
        ...
        warn("[DirectSyscall] failed");
        return EXIT_FAILURE;
    }

    okay("successfully injected into the payload with direct syscalls!");
...
int DirectSyscall(DWORD pid, PBYTE shell, SIZE_T shell_size) {
    ...
    if (hNtdll == INVALID_HANDLE_VALUE) {
        warn("handle to [NtDll] couldn't be procured");
        return 0;
    }
    okay("[0x%p] got a handle to NtDll", hNtdll);
